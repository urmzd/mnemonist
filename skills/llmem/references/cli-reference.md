...
| Flag | Description |
|------|-------------|
| `--root <path>` | Project root (default: current directory). Determines storage at `~/.llmem/{basename}/` |
| `--global` | Target global memory (`~/.llmem/global/`) instead of project |
...
### `llmem memorize "<point>" [-t type] [-n name] [--global] [--stdin]`
...
### `llmem remember "<ask>" [--budget N] [--level both] [--stdin]`
...
### `llmem learn [path] [--attend glob] [--capacity N]`
...
### `llmem consolidate [--dry-run] [--global]`
...
|------------|-------------|
| `init` | Create default `~/.llmem/config.toml` |
