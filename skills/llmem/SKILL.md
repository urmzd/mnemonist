...
name: llmem
description: >
  Persistent, tool-agnostic AI agent memory using the llmem CLI and file
  format. Use when managing memories, recalling context, ingesting code,
  running consolidation, or configuring llmem — even if the user just says
...
```bash
llmem init                    # create project memory
llmem init --global           # create global memory
llmem config init             # create default config.toml
```
...
llmem memorize "user is a senior backend engineer" -t user -n senior-backend
llmem memorize "deploy freeze until 2026-04-05" -t project --global
```

**Quick capture** — goes to working memory inbox:
```bash
llmem note "look into async runtime choices"
llmem note "might need to refactor auth module"
```

### 3. Ingesting Code

```bash
llmem learn .                           # ingest entire project
llmem learn src/ --attend "**/*.rs"     # only Rust files in src/
```
...
```bash
llmem remember "authentication"               # search both levels
llmem remember "rust patterns" --level project # project only
llmem remember "user preferences" --budget 500 # compact output
```
...
```bash
llmem consolidate --dry-run   # preview what would change
llmem consolidate             # run the full cycle
```
...
```bash
llmem reflect                 # project memories + inbox
llmem reflect --all           # both levels
llmem forget feedback_old.md  # remove a specific memory
```
...
Stderr carries colored UX text (only when TTY). This makes llmem pipe-friendly for agent integrations.
...
- `--root` determines which project directory under `~/.llmem/` is used — defaults to cwd basename
