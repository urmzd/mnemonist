...
use mnemonist_core::{
    EmbeddingStore, MemoryFile, MemoryIndex, global_dir, mnemonist_root, project_dir,
};
use mnemonist_index::{AnnIndex, hnsw::HnswIndex};
use serde::{Deserialize, Serialize};
use std::path::{Path, PathBuf};
use std::sync::{Arc, RwLock};

#[derive(Parser)]
#[command(name = "mnemonist-server", about = "Memory daemon for mnemonist")]
struct Cli {
...
async fn remember(
    ...
    let mut matched: Vec<(PathBuf, mnemonist_core::IndexEntry)> = Vec::new();
...
fn hnsw_search_from_state(
    ...
    dir: &Path,
    _query: &str,
    _matched: &mut Vec<(PathBuf, mnemonist_core::IndexEntry)>,
) {
...
fn text_search(dir: &Path, query: &str, matched: &mut Vec<(PathBuf, mnemonist_core::IndexEntry)>) {
...
async fn main() -> Result<()> {
    ...
    if let Some(ref addr) = cli.addr {
        ...
        println!("mnemonist-server listening on tcp://{addr}");
        ...
        println!("mnemonist-server listening on unix://{}", sock.display());
